...
(function($){
    $.fn.yxMobileSlider = function(settings){
        ...
        return this.each(function(){
            ...
            var winodww = $(window).width() * 0.5;
            $("div", oFocusContainer).width(oFocus.outerWidth(true) * num - $("span", oFocusContainer).height()).css({
                margin:'auto',
				overflow:'hidden'
            });
			...
			//解除绑定
			 function endTochuEvent(){
                oMover.get(0).removeEventListener('touchstart', touchStartFunc, false);
                oMover.get(0).removeEventListener('touchmove', touchMoveFunc, false);
                oMover.get(0).removeEventListener('touchend', touchEndFunc, false);
            }
            //获取触点位置
            function touchPos(e){
